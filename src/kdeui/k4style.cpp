...
/*
    ...
    QStyle/K4Style styleHint() or draw*() and (unconditionally) expect a valid return, however:
    ...
    Plain Qt dependent widgets can do that themselves and if a widget uses K4Style's convenience access
    ...
    objectName() of a const QWidget* - but this happens completely inside K4Style or the widget, if
    it does not make use of K4Styles static convenience functions.
...
QStyle::ControlElement K4Style::newControlElement(const QString &element)
{
    return (ControlElement)newStyleElement(element, "CE_", d->controlCounter, &d->styleElements);
}

K4Style::SubElement K4Style::newSubElement(const QString &element)
{
...
QStyle::StyleHint K4Style::customStyleHint(const QString &element, const QWidget *widget)
{
    return (StyleHint) customStyleElement(SH_KCustomStyleElement, element, const_cast<QWidget *>(widget));
}

QStyle::ControlElement K4Style::customControlElement(const QString &element, const QWidget *widget)
{
    return (ControlElement) customStyleElement(SH_KCustomStyleElement, element, const_cast<QWidget *>(widget));
}

QStyle::SubElement K4Style::customSubElement(const QString &element, const QWidget *widget)
{
    return (SubElement) customStyleElement(SH_KCustomStyleElement, element, const_cast<QWidget *>(widget));
}

void K4Style::polish(QWidget *w)
{
...
void K4Style::unpolish(QWidget *w)
{
    if (qobject_cast<QLabel *>(w)) {
...
void K4Style::polish(QApplication *a)
{
    QCommonStyle::polish(a);
}
void K4Style::unpolish(QApplication *a)
{
    QCommonStyle::unpolish(a);
}
void K4Style::polish(QPalette &pal)
{
    QCommonStyle::polish(pal);
}
QRect K4Style::itemTextRect(const QFontMetrics &fm, const QRect &r,
                            int flags, bool enabled,
                            const QString &text) const
{
    return QCommonStyle::itemTextRect(fm, r, flags, enabled, text);
}
QRect K4Style::itemPixmapRect(const QRect &r, int flags, const QPixmap &pixmap) const
{
    return QCommonStyle::itemPixmapRect(r, flags, pixmap);
}
void K4Style::drawItemText(QPainter *painter, const QRect &rect,
...
void K4Style::drawItemPixmap(QPainter *painter, const QRect &rect,
                             int alignment, const QPixmap &pixmap) const
{
    QCommonStyle::drawItemPixmap(painter, rect, alignment, pixmap);
}
QPalette K4Style::standardPalette() const
{
    return KColorScheme::createApplicationPalette(KSharedConfig::openConfig());
}

QIcon K4Style::standardIcon(StandardPixmap standardIcon, const QStyleOption */*option*/,
...
QPixmap K4Style::standardPixmap(StandardPixmap standardPixmap, const QStyleOption *opt,
                                const QWidget *widget) const
{
    return QCommonStyle::standardPixmap(standardPixmap, opt, widget);
}
QPixmap K4Style::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap &pixmap,
                                     const QStyleOption *opt) const
{
    return QCommonStyle::generatedIconPixmap(iconMode, pixmap, opt);
}

void K4Style::drawInsideRect(QPainter *p, const QRect &r) const
{
    p->drawRect(r.x(), r.y(), r.width() - 1, r.height() - 1);
}

QRect K4Style::centerRect(const QRect &in, int w, int h) const
{
    return QRect(in.x() + (in.width() - w) / 2, in.y() + (in.height() - h) / 2, w, h);
}

QRect K4Style::centerRect(const QRect &in, const QSize &size) const
{
    return centerRect(in, size.width(), size.height());
}

void K4Style::drawKStylePrimitive(WidgetType widgetType, int primitive,
                                  const QStyleOption *opt,
                                  const QRect &r, const QPalette &pal,
                                  State flags, QPainter *p,
                                  const QWidget *widget,
                                  K4Style::Option *kOpt) const
{
    switch (widgetType) {
    case WT_Tree: {
        switch (primitive) {
    ...
    case WT_Window: {
        ...
        case Window::ButtonMenu: {
            K4Style::TitleButtonOption *tbkOpts =
                extractOption<K4Style::TitleButtonOption *>(kOpt);
    ...
    if (primitive == Generic::Text) {
        K4Style::TextOption *textOpts = extractOption<K4Style::TextOption *>(kOpt);
        ...
        K4Style::IconOption *iconOpts = extractOption<K4Style::IconOption *>(kOpt);
        QIcon::Mode mode;
        ...
        //identical code!
        K4Style::ColorOption *colorOpt   = extractOption<K4Style::ColorOption *>(kOpt);
...
void K4Style::setWidgetLayoutProp(WidgetType widget, int metric, int value)
{
...
int K4Style::widgetLayoutProp(WidgetType widget, int metric,
                              const QStyleOption *opt,
...
QSize K4Style::expandDim(const QSize &orig, WidgetType wt, int baseMarginMetric,
...
QRect K4Style::insideMargin(const QRect &orig, WidgetType wt,
...
QRect K4Style::handleRTL(const QStyleOption *opt, const QRect &subRect) const
{
    return visualRect(opt->direction, opt->rect, subRect);
}

QPoint K4Style::handleRTL(const QStyleOption *opt, const QPoint &pos) const
{
    return visualPos(opt->direction, opt->rect, pos);
}

void K4Style::drawPrimitive(PrimitiveElement elem, const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
...
void K4Style::drawControl(ControlElement element, const QStyleOption *option, QPainter *p, const QWidget *widget) const
{
    //Extract the stuff we need out of the option
    State flags = option->state;
    QRect      r     = option->rect;
    ...
    */
    ...
    case CE_ProgressBarContents: {
        ...
        if (busyIndicator) {
            ...
            if (remSize <= 0) {
                remSize = 1;    //Do something non-crashy when too small...
            }
    ...
    case CE_MenuItem: {
        ...
        if (!miOpt || miOpt->menuItemType == QStyleOptionMenuItem::EmptyArea) {
            return;
        }
...
int K4Style::styleHint(StyleHint hint, const QStyleOption *option, const QWidget *widget, QStyleHintReturn *returnData) const
{
    switch (hint) {
...
int K4Style::pixelMetric(PixelMetric metric, const QStyleOption *option, const QWidget *widget) const
{
    switch (metric) {
...
int K4Style::layoutSpacing(QSizePolicy::ControlType control1, QSizePolicy::ControlType control2, Qt::Orientation orientation, const QStyleOption *option, const QWidget *widget) const
{
...
bool K4Style::isVerticalTab(const QStyleOptionTab *tbOpt) const
{
    switch (tbOpt->shape) {
...
bool K4Style::isReflectedTab(const QStyleOptionTab *tbOpt) const
{
    switch (tbOpt->shape) {
    case QTabBar::RoundedEast:
    case QTabBar::TriangularEast:
...
K4Style::Side K4Style::tabSide(const QStyleOptionTab *tbOpt) const
{
    switch (tbOpt->shape) {
    case QTabBar::RoundedEast:
    case QTabBar::TriangularEast:
...
QRect K4Style::marginAdjustedTab(const QStyleOptionTab *tabOpt, int property) const
{
    QRect r = tabOpt->rect;
...
bool K4Style::useSideText(const QStyleOptionProgressBar *pbOpt) const
{
    ...
    if (!pbOpt->textVisible) {
        return false;    //Don't allocate side margin if text display is off...
    }

    if (pbOpt->textAlignment & Qt::AlignHCenter) {
        return false;    //### do we want this? we don't
    }
    //force indicator to the side outside
    //the main otherwise.

    if (pbOpt->minimum == pbOpt->maximum) {
        return false;
    }
...
int K4Style::sideTextWidth(const QStyleOptionProgressBar *pbOpt) const
{
    return pbOpt->fontMetrics.width(QLatin1String("100%")) +
           2 * widgetLayoutProp(WT_ProgressBar, ProgressBar::SideTextSpace);
}

QRect K4Style::subElementRect(SubElement sr, const QStyleOption *option, const QWidget *widget) const
{
    QRect r = option->rect;
...
void  K4Style::drawComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
...
QRect K4Style::internalSubControlRect(ComplexControl control, const QStyleOptionComplex *option,
...
QRect K4Style::subControlRect(ComplexControl control, const QStyleOptionComplex *option,
                              SubControl subControl, const QWidget *widget) const
{
    QRect r = option->rect;
...
QStyle::SubControl K4Style::hitTestComplexControl(ComplexControl cc, const QStyleOptionComplex *opt,
...
QSize K4Style::sizeFromContents(ContentsType type, const QStyleOption *option, const QSize &contentsSize, const QWidget *widget) const
{
    switch (type) {
    case CT_PushButton: {
        const QStyleOptionButton *bOpt = qstyleoption_cast<const QStyleOptionButton *>(option);
    ...
    case CT_MenuItem: {
        const QStyleOptionMenuItem *miOpt = ::qstyleoption_cast<const QStyleOptionMenuItem *>(option);
        if (!miOpt) {
            return contentsSize;    //Someone is asking for trouble..
        }
...
bool K4Style::eventFilter(QObject *obj, QEvent *ev)
{
...
K4Style::ColorMode::ColorMode(Mode _mode, QPalette::ColorRole _role):
    mode(_mode),
    role(_role)
{}
...
K4Style::TextOption::TextOption()
{
    init();
}

K4Style::TextOption::TextOption(const QString &_text):
    text(_text)
{
    init();
}

void K4Style::TextOption::init()
{
