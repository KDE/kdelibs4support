...
template <>
inline void KConfigGroup::writeEntry(const char *key, const KUrl &value, KConfigBase::WriteConfigFlags flags)
{
    writeEntry(key, value.url(), flags);
}

template <>
inline KUrl KConfigGroup::readEntry(const char *key, const KUrl &defaultValue) const
{
